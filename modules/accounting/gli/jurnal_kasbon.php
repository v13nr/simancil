...
//		$("#norek2").change(onSelectChange2);		
  ...
  <tr>
    ...
    <td width="1090"><span class="style1">JURNAL 
      KAS KELUAR UNTUK PIUTANG KARYAWAN</span>
      <hr /></td>
  </tr>
  <tr>
    <td width="32">&nbsp;</td>
	...
	<form name="frmijin" id="frmijin" method="post" action="submission_gli.php">
          ...
          <input type="hidden" name="tipe_jurnal" value="JKB" />
          <input type="hidden" name="khusus" value="kasbon" />
      ...
      <tr>
        ...
        <td><select name="dk" id="dk" class="required" title="*">
		 <?php  if ($_GET['dk']=="Kredit"){?> 
		 <option value="Kredit" <?php  if ($_GET['dk']=="Kredit"){?> selected="selected" <?php  }?>>Pemberian Pinjaman</option>
         <?php  } elseif ($_GET['dk']=="Debet"){?> 
		 <option value="Debet" <?php  if ($_GET['dk']=="Debet"){?> selected="selected" <?php  }?>>Penerimaan Angsuran</option>
		 <?php  } else { ?>
      ...
      <tr>
        <td>No. Perkiraan </td>
        <td><input type="text" name="norek" id="norek" readonly="true" maxlength="8" size="10" class="required" title="*" value="AL1-1111" <?php  if($_GET['norek']<>""){?> <?php  } ?> />
		<?php  if($_GET['norek']==""){?>
		<?php  } ?>
      ...
      <tr>
        <td>Keterangan </td>
        <td><input type="text" name="keteranganheader" size="50" class="required" readonly="true" title="*" value="Kas"  <?php  if($_GET['keteranganheader']<>""){?>  <?php  } ?>></td>
      </tr>
      <tr>
        <td>TOTAL</td>
        <td>
	...
	<table border="1" align="left" bordercolor="#FFFFFF" cellpadding="3" cellspacing="0" bordercolorlight="silver" bordercolordark="#FFFFFF">
      ...
      <tr bgcolor="yellow">
          ...
          <td align="center"><select name="norek2" id="norek2" class="required" title="*" >
            <?php 
				$SQL = "SELECT * FROM $database.rekening WHERE substr(norek, -4) <> '0000' and (norek LIKE 'AL2-1113') ORDER BY norek";
				$hasil = mysql_query($SQL, $dbh_jogjaide) or die(mysql_error());
				while($baris = mysql_fetch_array($hasil)){
							?>
					<option value="<?php  echo $baris['norek']?>" selected="selected">
	...
	?>
      <tr <?php 	 if (($nRecord % 2)==0) {?>bgcolor="#FFFFFF"<?php  } else {?> else="else" bgcolor="#CCCCCC"<?php  }?>>
        <form action="submission_gli.php" method="post" name="frmijin" id="frmijin">
          ...
          <input type="hidden" name="cmd" value="upd_jurnal_kasbon" />
          ...
          <td align="center"><a href="javascript:confirmDelete('submission_gli.php?id=<?php  echo $row["id"]?>&amp;cmd=del_jurnal_kasbon&nobukti=<?php  echo $_GET['nobukti']?>&tgl_transaksi=<?php  echo $_GET['tgl_transaksi']?>&dk=<?php  echo $_GET['dk']?>&norek=<?php  echo $_GET['norek']?>&namarek=<?php  echo $_GET['namarek']?>&divisi=<?php  echo $_GET['divisi']?>&bulan=<?php  echo $_GET['bulan']?>&keteranganheader=<?php  echo $_GET['keteranganheader']?>')"><img src="../images/hapus.gif" alt="Hapus" border="0" /></a></td>
          <?php  } ?>
        </form>
      </tr>
      <?php   
		...
		<tr>
			...
			<a href="jurnal_kasbon.php">[ SELESAI ATAU KE NOMOR BUKTI BERIKUTNYA ]</a>
