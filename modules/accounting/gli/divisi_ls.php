      ...
      <tr <?php 	 if (($nRecord % 2)==0) {?>bgcolor="#FFFFFF"<?php  } else {?> else="else" bgcolor="#CCCCCC"<?php  }?>>
        <form action="submission_gli.php" method="post" name="frmijin" id="frmijin">
          ...
          <td align="center"><?php  if ($_GET['id']<>"") { ?>
            <input type="text" name="subdiv" size="20" class="required" title="*" maxlength="2" value="<?php  echo $row['subdiv']?>">
            <?php  } else { ?>
              <?php  echo $row["subdiv"]?>
              <?php  } ?>
          </td>
          <td align="left"><?php  if ($_GET['id']<>"") { ?>
            <input type="text" name="namadiv" size="50" class="required" title="*"  value="<?php  echo $row['namadiv']?>">
            <?php  } else { ?>
              <?php  echo $row["namadiv"]?>
              <?php  } ?></td>
          <?php  if ($_GET['id']<>"") { ?>
          ...
          <?php  } else { ?>
          <td align="center"><a href="?mn=<?php  echo $_GET['mn']?>&amp;id=<?php  echo $row["subdiv"]?>"><img src="../images/edit.gif" alt="Edit" border="0" /></a></td>
          <td align="center"><a href="javascript:confirmDelete('submission_gli.php?id=<?php  echo $row["subdiv"]?>&amp;cmd=del_divisi')"><img src="../images/hapus.gif" alt="Hapus" border="0" /></a></td>
          <?php  } ?>
        </form>
      </tr>
      <?php   
