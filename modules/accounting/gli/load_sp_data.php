<?php
	...
	if(isset($_POST['data_type']) && isset($_POST['parent_id'])){
		...
		switch($data_type){
			case 'produk': 
				$sql = "SELECT kodebrg id, namabrg nama FROM $database.stock WHERE status = 1 AND divisi = '$parent_id'"; break;
			case 'subcarabayar': 
				$sql = "SELECT kode id, nama FROM $database.supplier WHERE divisi = '$parent_id'"; break;
			case 'carabayar':
			default:
