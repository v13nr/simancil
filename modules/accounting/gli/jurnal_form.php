<? session_start(); ?>
<? include "otentik_gli.php"; include ("../include/globalx.php"); 
...
$(document).ready(function() {
  ...
  <tr>
	...
	<form name="frmijin" id="frmijin" method="post" action="submission_gli.php">
      ...
      <tr>
        <td>No. Perkiraan </td>
        <td><input type="text" name="norek" id="norek" readonly="true" maxlength="8" size="10" class="required" title="*" value="<?=$_GET['norek']?>" <? if($_GET['norek']<>""){?> <? } ?> />
		<? if($_GET['norek']==""){?>
		...
		<? } ?>
		<input type="hidden" name="namarek" size="30" value="<?=$_GET['namarek']?>" readonly="true" />        </td>
      </tr>
      <tr>
        <td>Keterangan </td>
        <td><input type="text" name="keteranganheader" size="50" class="required" title="*" value="<?=$_GET['keteranganheader']?>"  <? if($_GET['keteranganheader']<>""){?>  <? } ?>></td>
      </tr>
      <tr>
        <td>TOTAL</td>
        <td>
