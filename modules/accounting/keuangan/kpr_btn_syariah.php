...
<table width="90%" border="1" style="border-collapse:collapse">
  <tr>
    ...
    <td colspan="6"><div align="center">USER KPR BTN SYARIAH </div></td>
    <td>&nbsp;</td>
  ...
  <?php 
	...
	if(isset($_GET['id'])){
		$sql = mysql_query("SELECT * FROM kpr_btnsy WHERE id = '".$_GET['id']."' ORDER BY nama");
  ...
  <tr>
	...
	<input name="cmd" type="hidden" value="<?php  echo isset($_GET['id']) ? 'upd_kpr_btnsy' : 'add_kpr_btnsy'; ?>">
    ...
    <td><input type="text" name="persen5" value="<?php  echo $data['persen5']; ?>" size="12" /></td>
    <td><input type="text" name="total" value="<?php  echo $data['total']; ?>" size="12" /></td>
	...
	<?php 
		$sql = mysql_query("SELECT * FROM kpr_btnsy ORDER BY nama");
		$no = 1;
	...
	?>
		...
		<tr>
			...
			<td align="right"><?php  echo number_format($data['persen5']); $terhold1 = $terhold1 + $data['persen5'];?></td>
			<td align="right"><?php  echo number_format($data['total']); $terhold2 = $terhold2 + $data['total']; ?></td>
			<td><?php  echo (baliktglindo($data['tanggal_akad'])  != '00-00-0000') ? baliktglindo($data['tanggal_akad']) : '';?></td>
			<td>
				<a href="submission_keu.php?cmd=del_kpr_btnsy&id=<?php  echo $data['id']; ?>" onclick="return confirm('Apakah anda yakin ?')">Delete</a>
				<a href="?mn=kpr_btnsy&id=<?php  echo $data['id']; ?>">Edit</a>			</td>
		</tr>
	<?php 
			$no++;
		}
