<?php
	...
	if(isset($_GET['id'])){
		$id = $_GET['id'];
		$sql = mysql_query("SELECT * FROM nas_produksi.tk WHERE kode = '$id'");
...
<?php	
		...
		if(isset($_GET['submitSearch'])){
			$search = $_GET['search'];
			$SQL = "SELECT * FROM nas_produksi.tk WHERE nama LIKE '%$search%' AND status = 1 AND id_divisi = '$idDivisi' ORDER BY nama LIMIT $offset,$rs_PerHal";
			$datas = mysql_query($SQL, $dbh_produksi) or die(mysql_error($SQL));		
		}	
		else
			$SQL = "SELECT * FROM nas_produksi.tk WHERE status = 1 AND id_divisi = '$idDivisi' ORDER BY nama LIMIT $offset,$rs_PerHal";
			//echo $SQL;
				...
				<div class="table">
					<table>
						...
						<input type="hidden" name="cmd" value="add_tk" />
							<tr align="center">
								<td><img src="images/kal_next.gif" alt="Selanjutnya" border="0" /></td>
								<td><input type="text" name="kode" maxlength="4"></td>
								<td>
					...
					?>
							...
							<div class="pagination pagination-left">
								<div class="results">
									<?php
										if(isset($_GET['submitSearch']))
											$rekening = mysql_query("SELECT nama FROM nas_produksi.tk WHERE nama LIKE '%$search%' AND id_divisi = '$idDivisi'");
										else
											$rekening = mysql_query("SELECT nama FROM nas_produksi.tk WHERE id_divisi = '$idDivisi'");	
								...
								<ul class="pager">
									<?php
										if(isset($_GET['submitSearch'])){
											$query = "SELECT COUNT(*) AS rs_Jumlah FROM tk WHERE nama LIKE '%$search%' AND id_divisi = '$idDivisi'";
											//$link = 
											...
											$query = "SELECT COUNT(*) AS rs_Jumlah FROM nas_produksi.tk WHERE id_divisi = '$idDivisi'";
											//$link = 
