...
<?php	
		...
		if(isset($_GET['submitSearch']) && $_GET['search']<> ""){
			...
			$SQL = "SELECT * FROM nas_produksi.produksi WHERE status = 1 AND id_divisi = '$idDivisi'  ";
			if(isset($_GET['bulan']) && isset($_GET['tahun'])){
				$SQL = $SQL . " and MONTH(tanggal) = '". $_GET["bulan"] ."' and YEAR(tanggal) = '". $_GET["tahun"] ."'";
			}
			$SQL = $SQL . " ORDER BY tanggal LIMIT $offset,$rs_PerHal";
			//echo $SQL;
