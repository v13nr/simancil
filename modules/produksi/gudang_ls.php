...
<?php 
	$SQL = "SELECT * FROM nas_produksi.gudang WHERE kode <> ''";
	if ($_GET['id']<>"") {
...
<table width="1140" border="0">
  <tr>
    ...
    <td width="1090"><span class="style1">SETUP GUDANG 
      </span>
      <hr /></td>
  </tr>
  <tr>
    ...
    <td><table border="1" align="left" cellpadding="3" cellspacing="0" bordercolorlight="silver" bordercolordark="#FFFFFF">
      ...
      <?php  } else { ?>
      <tr>
        <td background="../../images/impactg.png" colspan="9" align="center"><strong><font color="white">MASTER GUDANG </font></strong></td>
      </tr>
      <?php  } ?>
      <tr bgcolor="#FFCC00">
      ...
      <tr bgcolor="yellow">
        <form name="frmijin" id="frmijin" method="post" action="submission_produksi.php">
          <input type="hidden" name="cmd" value="add_gudang" />
	...
	?>
      <tr <?php 	 if (($nRecord % 2)==0) {?>bgcolor="#FFFFFF"<?php  } else {?> else="else" bgcolor="#CCCCCC"<?php  }?>>
        <form action="submission_produksi.php" method="post" name="frmijin" id="frmijin">
          ...
          <input type="hidden" name="cmd" value="upd_gudang" />
          ...
          <td align="center"><a href="javascript:confirmDelete('submission_produksi.php?id=<?php  echo $row["kode"]?>&amp;cmd=del_gudang')"><img src="../../images/hapus.gif" alt="Hapus" border="0" /></a></td>
          <?php  } ?>
        </form>
      </tr>
      <?php   
