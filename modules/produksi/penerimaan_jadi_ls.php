...
?>
		<div id="content">
			...
			<div class="box">
				<!-- box / title -->
				<div class="title">
					<h5>PENERIMAAN BARANG JADI / PRODUKSI </h5>
				</div>
				<!-- end box / title -->
				<div class="table">
					<table>
						...
						<tbody>
							<form action="modules/produksi/submission_produksi.php" method="post" id="formRekening">
								...
								<input type="hidden" name="cmd" value="upd_pjadi" />
								<tr align="center">
									...
									<td>
										<input type="text" name="terima" value="<?php echo $data['terima']; ?>" style="width: 30px;">
									</td>
									<td>
				...
				<div class="table">
					<table>
						...
						<input type="hidden" name="cmd" value="add_pjadi" />
							<tr align="center">
								<td><img src="images/kal_next.gif" alt="Selanjutnya" border="0" /></td>
								<td><input type="text" name="tanggal"  id="datepicker" maxlength="10"></td>
								<td>
									<input type="text" name="terima" style="width: 30px;">
								</td>
								<td>
									<select name="unit" class="tes">
							...
							?>
										<tr align="center">
											...
											<td><a href="index.php?mn=penerimaan_jadi_ls&getmodule=<?php echo base64_encode('produksi/');?>&amp;id=<?php echo $data['id']; ?>" title="Edit"><img src="resources/images/edit.png" /></a></td>
											<td><a href="javascript:confirmDelete('modules/produksi/submission_produksi.php?id=<?php echo $data['id']; ?>&amp;cmd=del_pjadi')" title="Hapus"><img src="resources/images/delete.gif" /></a></td>
										</tr>						
							<?php
										$no++;
									}
								...
								<ul class="pager">
									<?php
										if(isset($_GET['submitSearch'])){
											...
											//$link = 
											$prev = 'index.php?mn=penerimaan_jadi_ls&getmodule='.base64_encode('produksi/').'&submitSearch=true&search='.$search.'&hal='.($noHal-1);
											$next = 'index.php?mn=penerimaan_jadi_ls&getmodule='.base64_encode('produksi/').'&submitSearch=true&search='.$search.'&hal='.($noHal+1);
										} else{
											$query = "SELECT COUNT(*) AS rs_Jumlah FROM nas_produksi.produksi WHERE id_divisi = '$idDivisi'";
											//$link = 
											$prev = 'index.php?mn=penerimaan_jadi_ls&getmodule='.base64_encode('produksi/').'&hal='.($noHal-1);
											$next = 'index.php?mn=penerimaan_jadi_ls&getmodule='.base64_encode('produksi/').'&hal='.($noHal+1);
										}
										...
										for($hal = 1; $hal <= $jumPage; $hal++)
										{
										  ...
										  {   
											...
											$link = isset($_GET['submitSearch']) ? 'index.php?mn=penerimaan_jadi_ls&getmodule='.base64_encode('produksi/').'&submitSearch=true&search='.$search.'&hal='.$hal : 'index.php?mn=penerimaan_jadi_ls&getmodule='.base64_encode('produksi/').'&hal='.$hal; 
