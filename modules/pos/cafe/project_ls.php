...
//		$SQL = $SQL." ORDER BY norek ASC";
    ...
    <tr <?	 if (($nRecord % 2)==0) {?>bgcolor="#e4e4e4"<? }  else {?>bgcolor="#FFFFCC"<? } ?>>
      ...
      <td class="style3" align="let"><? echo "PR/".nobukti($row['nomor']);?></td>
	  ...
	  { ?>
	  <a href="pemesanan_project_rinci.php?project_id=<?=$row['id']?>&nomor=<?=$row['nomor']?>&sub=<?=$row['sub']?>&tgl_transaksi=<?=baliktglindo($row['tgl'])?>"><img src="../draft/images/user_go.png" border="0" width="16" height="16"></a>
	  <? } ?>
	  </div></td>
    </tr>
	<?  
