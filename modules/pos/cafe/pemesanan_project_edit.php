  ...
  <tr>
	...
	<form name="frmijin" id="frmijin" method="post" action="submission_cafe.php">
      ...
      <tr>
        ...
        <td width="205"><input type="text" name="nonota" value="PR/<?php  echo nobukti($_GET['nomor'])?>" readonly="true" /></td>
	...
	<?php  $id_posting = $row["id"]?>
      <tr <?php 	 if (($nRecord % 2)==0) {?>bgcolor="#FFFFFF"<?php  } else {?> else="else" bgcolor="#CCCCCC"<?php  }?>>
		  ...
		  <a class="posted_url" href="javascript:confirmPOS('submission_inv.php?id=<?php  echo $id_posting;?>&harga=<?php  echo $row["harga"]?>&qty=<?php  echo $row["qtyin"]?>&cmd=add_to_inv&nonota=PO/<?php  echo nobukti($_GET['nomor'])?>&supp=<?php  echo $_GET['supp']?>&alamat=<?php  echo $_GET['alamat']?>&kota=<?php  echo $_GET['kota']?>&
      ...
      <?php   
		...
		<tr>
			...
			<a href="popup_bayar.php?width=500&height=300&nonota=<?php  echo $_GET['nomor']?>&TB_iframe=true" id="cari" class="thickbox" title="Pembayaran">[ CETAK ]</a>&nbsp;<a href="pemesanan_project.php">[ SELESAI ATAU KE NOMOR PR BERIKUTNYA ]</a>
		  </td>
		</tr>
		<?php 
	} else { ?>
