  ...
  <tr>
	...
	<form name="frmijin" id="frmijin" method="post" action="submission_cafe.php">
      ...
      <tr>
		...
		?>
		<option value="<?=$baris["id"];?>" <?php if($_GET["meja"]== $baris["id"]){?> selected="selected" <? }?>><?=$baris["nama"];?></option>
		<? } ?>
		</select>
		</td>
        <td>Shift</td>
	...
	?>
      ...
      <?  
		...
		<tr>
			...
			<a href="popup_bayar.php?width=500&height=300&nonota=<?=$_GET['nomor']?>&TB_iframe=true" id="cari" class="thickbox" title="Pembayaran">[ CETAK ]</a>&nbsp;<a href="index.php?mn=penjualan">[ SELESAI ATAU KE NOMOR INV BERIKUTNYA ]</a>
		  </td>
		</tr>
		<?
	} else { ?>
