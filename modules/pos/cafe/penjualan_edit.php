...
function hitung(){
  ...
  <tr>
    ...
    <td width="1090"><span class="style1">PEMAKAIAN / BARANG KELUAR : 
        <?php  echo $_GET['pesan_id']?> </span>
      <hr /></td>
  </tr>
  <tr>
    <td width="32">&nbsp;</td>
	...
	<form name="frmijin" id="frmijin" method="post" action="submission_cafe.php">
      ...
      <tr>
		...
		?>
		<option value="<?php  echo $baris["id"];?>" <?php  if($_GET["meja"]== $baris["id"]){?> selected="selected" <?php  }?>><?php  echo $baris["nama"];?></option>
		<?php  } ?>
		</select>
		</td>
        <td>Shift</td>
	...
	?>
      ...
      <?php   
		...
		<tr>
			...
			<a href="popup_bayar.php?width=500&height=300&nonota=<?php  echo $_GET['nomor']?>&TB_iframe=true" id="cari" class="thickbox" title="Pembayaran">[ CETAK ]</a>&nbsp;<a href="index.php?mn=penjualan">[ SELESAI ATAU KE NOMOR INV BERIKUTNYA ]</a>
		  </td>
		</tr>
		<?php 
	} else { ?>
