<?php 
 ...
 switch($_GET['cari']){
	case "rekening" :
		...
		if ($_GET['mode']=="rekp") {
 			$query = mysql_query("SELECT namarek FROM $database.rekening WHERE norek = '$id'", $dbh_jogjaide);
		}
		break;
	case "rumah" :
		if ($_GET['mode']=="kpr") {
 			$query = mysql_query("SELECT kpr FROM $database.stock WHERE kodebrg = '$id'", $dbh_jogjaide);
		}
		break;
	case "supplier" :
		if($_GET['mode']=="nama"){
