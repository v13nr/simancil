...
<?
 ...
 $SQL = "SELECT * FROM rek WHERE norek <> '' ORDER by norek";
 if(isset($_POST['search'])){
	 $SQL = "SELECT * FROM rek WHERE namarek LIKE '%$_POST[search_name]%' ORDER by norek";
 }
...
<table width="100%" bgcolor="#000000" cellspacing="1" cellpadding="3">	
	...
	<tr bgcolor="#FFFFFF">
		...
		<td align="center"><a href="javascript:selectBuku('<?=$row->norek?>','<?=$row->namarek?>','<?=$row->tipe?>')"><?=$row->norek?></a></td>
		<td><?=$row->namarek?></td>
