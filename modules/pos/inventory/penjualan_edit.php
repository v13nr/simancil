...
//		$query = mysql_query("SELECT * FROM rek a, konsumen b WHERE a.norek = b.norek AND b.kode = '$kodespp'");
	...
	<form name="frmijin" id="frmijin" method="post" action="submission_inv.php">
      ...
      <tr>
        ...
        <td width="255"><input type="text" name="nonota" value="IDN/<?=nobukti($_GET['nomor'])?>" readonly="true" /></td>
        <td width="115">Tanggal</td>
        <td width="347"><input type="text" name="tgl_transaksi" id="tgl_transaksi" size="10" class="required" title="*" value="<?=baliktglindo($tgl)?>" readonly="true" /></td>
      </tr>
      <tr>
        <td>Divisi</td>
        <td><?=$divisi?></td>
        ...
        <td><input type="text" name="angsuran" id="angsuran" size="10" class="required" title="*" value="<?php echo $angsuran; ?>" <? if($_GET['angsuran']<>""){ ?>  readonly="true"  <? }?> />
          Bunga (%) / Tahun 
            <input type="text" name="bunga" id="bunga" size="5" class="required" title="*" value="<?php echo $bunga ?>" <? if($_GET['angsuran']<>""){ ?>  readonly="true"  <? }?> /></td>
      </tr>
      <tr>
        <td>Konsumen </td>
        <td>
...
:
  <input type="text" name="namasupp" value="<?=$namaspp?>" id="namasupp" readonly="true" /></td>
      ...
      <tr>
        ...
        <td>Salesi </td>
        <td><input type="text"  id="sales" name="sales"  class="required kanan" value="<?=$sales?>" />
&nbsp; Kode
<input type="text" id="kode_sales" name="kode_sales" size="10"  class="required kanan" value="<?=$kode_sales?>"/></td>
      </tr>
    </table>
	<br />
	...
	?>
      <tr <?	 if (($nRecord % 2)==0) {?>bgcolor="#FFFFFF"<? } else {?> else="else" bgcolor="#CCCCCC"<? }?>>
        <form action="submission_inv.php" method="post" name="frmijin" id="frmijin">
		  ...
		  <td colspan="2" align="right"><?=number_format($row["dpx"],2,'.',',');?></td>
          ...
          <td align="center"><a href="javascript:confirmDelete('submission_inv.php?id=<?=$row["id"]?>&cmd=del_jual&cmd2=edit&nonota=<?=$_GET['nonota']?>&supp=<?=$_GET['supp']?>&alamat=<?=$_GET['alamat']?>&kota=<?=$_GET['kota']?>&telp=<?=$_GET['telp']?>&tgl_transaksi=<?=$_GET['tgl_transaksi']?>&saldo=<?=$_GET['saldo']?>&rek=<?=$_GET['rek']?>&namarek=<?=$_GET['namarek']?>&nomor=<?=$_GET['nomor']?>&namasupp=<?=$_GET['namasupp']?>&brg=<?=$row['kodebrg']?>&qtyout=<?=$row['qtyout']?>&netto=<?=$row["kredit"]?>&sub=<?=$_GET['sub']?>')">
      ...
      <?  
		...
		<tr>
			...
			<a href="cetak_penjualan.php?model=IDN&nomor=<?=$_GET['nomor']?>">[ CETAK ]</a>&nbsp;<a href="penjualanRumah.php">[ SELESAI ATAU KE NOMOR IDN BERIKUTNYA ]</a>		  </td>
		</tr>
		<?
	} else { ?>
      <tr>
