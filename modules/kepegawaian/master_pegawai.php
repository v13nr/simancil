...
<table  class="style3" cellspacing="1" bgcolor="#000000">
	...
	<tr bgcolor="#FFCC00">
		<td align="center"><input type="text" name="c_no" size="15" value="<?=$_GET['c_no']?>" /></td>
		<td><input type="text" name="c_nama" value="<?=$_GET['c_nama']?>" /></td>
		<td>
			<select name="c_jk">
				<option value="">-ALL-</option>
				<option value="L" <? if($_GET['c_jk']=="L") {?> selected="selected" <? }?>>Laki-Laki</option>
				<option value="P" <? if($_GET['c_jk']=="P") {?> selected="selected" <? }?>>Perempuan</option>
			</select>		</td>
		<td><select name="c_dep">
          ...
          <? 
			...
			?>
          <option value="<?=$r['iddep']?>" <? if ($_GET['c_dep']==$r['iddep']) {?>selected="selected"<? } ?>>
          <?=$r['namadept']?>
          </option>
          <? } ?>
        </select>		</td>
...
<input type="hidden" name="cmd" value="del_peg" />
  <table width="1024" border="0" cellspacing="1" class="style3">
    <tr>
      ...
      <td width="50" rowspan="3" valign="bottom"><div align="center"><a href="export.php?c_no=<?=$_GET['c_no']?>&c_jk=<?=$_GET['c_jk']?>&c_nama=<?=$_GET['c_nama']?>&c_dep=<?=$_GET['c_dep']?>"><img src="../../images/fileex.png" border="0" width="32" height="32" /></a></div></td>
...
$oPagination = new phpPagination ($nTotalItems, $nItemsPerPage);
    ...
    <tr <?	 if (($nRecord % 2)==0) {?>bgcolor="#e4e4e4"<? }  else {?>bgcolor="#FFFFCC"<? } ?>>
      <td height="76" align="center" class="style3"><?=++$No + (($nCurrentPage -1 ) * 10)?></td>
	  <td class="style3" align="center"><input type="checkbox" id="tambah" name="tambah[]" value="<?=$row['idno'] ?>" /></td>
      <td class="style3" align="center"><a href='javascript:PopUp("upload.php?idusr=<?=$row['idno']?>&modx=ok")'><img src="foto/<?=$row['foto']?>" width="61" /></a></td>
	  <td class="style3" align="center"><?=$row['noinduk']?></td>
      <td class="style3"><?=$row['nama']?></td>
      <td class="style3"><?=$row['alamat']?></td>
	  <td class="style3" align="center"><?=$row['notelp']?></td>
      <td class="style3" align="center"><?=baliktglindo($row['tgllahir'])?></td>
      <td class="style3" align="center"><?=$row['jkel']?></td>
      ...
      <td class="style3"><div align="center"><a href="input_pegawai.php?id=<?=$row['idno'] ?>"><img src="../../images/user_go.png" border="0" width="16" height="16"></a></div></td>
    </tr>
	<?  
		 $nRecord = $nRecord + 1;
		} 
