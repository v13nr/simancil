...
<?php 
	$tsql0 = "select * from armada where id <> ''";
	if ($_GET['iddep']<>"") {
...
<table width="1140" border="0">
  <tr>
    ...
    <td width="1098"><span class="style1">DATA ANGKUTAN</span>
      <hr /></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td><table border="1" width="60%" bordercolorlight="#FFFFFF" cellspacing="0" cellpadding="3" bordercolordark="#FFFFFF" align="left">
      <?php  if ($_GET['iddep']<>"") {?>
      <tr>
        <td  background="../../images/impactg.png" height="30" colspan="9" align="center"><font color="white"><b>EDIT DATA ANGKUTAN</b></font></td>
      </tr>
      <?php  } else { ?>
      <tr>
        <td  background="../../images/impactg.png" height="30" colspan="9" align="center"><font color="white"><b> DATA ANGKUTAN </b></font> </td>
      </tr>
      <?php  }?>
      <tr bgcolor="#FFCC00">
	...
	if ($hasil) { 
      ...
      <tr <?php 	 if (($nRecord % 2)==0) {?>bgcolor="#FFFFCC"<?php  } else{ ?>bgcolor="#E4E4E4" <?php  } ?>>
        <form method="post" action="expedisi_submission.php">
          ...
          <input type="hidden" name="cmd" value="upd_armada" />
          <td align="right"><div align="center">
            <?php  echo $nRecord?>
          </div></td>
          <td align="left"><?php  if ($_GET['iddep']<>"") { ?>
              <input type="text" name="layanan" size="40" class="form_isian" value="<?php  echo $row["keterangan"];?>" />
              <?php  } else { ?>
              <?php  echo $row["keterangan"]?>
              <?php  } ?>          </td>
          <td align="center"><?php  if ($_GET['iddep']<>"") { ?>
            <input type="text" name="satuan" size="15" class="form_isian" value="<?php  echo $row["nopol"];?>" />
            <?php  } else { ?>
            <?php  echo $row["nopol"]?>
            <?php  } ?></td>
          <td align="center"><?php  if ($_GET['iddep']<>"") { ?>
            <input type="text" name="harga" size="25" class="form_isian" value="<?php  echo $row["sopir"];?>" />
            <?php  } else { ?>
            <?php  echo ($row["sopir"])?>
            <?php  } ?></td>
          <?php  if ($_GET['iddep']<>"") { ?>
          ...
          <?php  } else { ?>
          <td align="center"><a href="sopir.php?iddep=<?php  echo $row["id"]?>"><img src="../../images/edit.gif" alt="Edit" border="0" /></a></td>
          <td align="center"><a href="javascript:confirmDelete('expedisi_submission.php?id=<?php  echo $row["id"]?>&amp;cmd=del_armada')"><img src="../../images/hapus.gif" alt="Hapus" border="0" /></a></td>
          <?php  } ?>
        </form>
      </tr>
      <?php   
      ...
      <tr bgcolor="yellow">
        <form method="post" action="expedisi_submission.php">
          <input type="hidden" name="cmd" value="add_armada" />
